...
import time
import tqdm as tq
import read_excel_drink_list as redl


#drink_list = ['사이다', '콜라']
drink_list = redl.get_drink_dic()
...
def add():
    ...
    if drink_name is not None and drink_name.strip() != "":
        ...
        if drink_name in drink_list:
            msg_form("[실패] 이미 등록되어있는 음료입니다.")
            delay_time()
        # 음료 중복 없을 시
        else:
            try:
                drink_price = int(input('음료 가격을 입력하시오 : '))
                drink_list[drink_name] = drink_price
                msg_form("[성공] 새로운 음료 등록 완료")
            except ValueError:
                msg_form("[실패] 입력중 오류가 발생하였습니다. ")
                delay_time()
...
def delete():
    try:
        drink_name = input('삭제할 음료 이름를 입력하시오 : ')

        # 딕셔너리에서 key 값으로 value 삭제
        drink_list.pop(drink_name)

        msg_form("[성공] 음료가 제거되었습니다.")
        delay_time()
    except KeyError:
        msg_form("[실패] 메뉴판에 없는 음료")
        delay_time()
...
def print_menu():
    print('\n\n')
    print('# 자판기 메뉴판 # ')
    if len(list(drink_list.keys())) == 0:
        print('현재 등록된 음료가 없습니다.')
    else:
        count = 0
        for key, value in drink_list.items():
            count += 1
            print("{num}.{name} : {price}원 ".format(num=count, name=key, price=value))
        print()
